#!/usr/bin/env python3
"""Minimal audited demand preparation for n=29, Delta=16.

No project imports.  Uses only:
  * the exact charging domain;
  * residual-activity bounds on r;
  * the audited threshold inequality 2 W_h <= z_h^2-z_h+h(h+1);
  * an exact Hall/source-capacity dual certificate.
The older pair_capacity support formula is deliberately not used.
"""
from fractions import Fraction
from math import comb,lcm
from pathlib import Path
import argparse,json
from scipy.optimize import linprog

def bounds(s,b,t):
    a=len(s)
    charge=sum((Fraction(x*(x-1),a-x) for x in s),Fraction())
    lo=b+(charge.numerator+charge.denominator-1)//charge.denominator
    hi=min(sum(s)-2*t,comb(a,2)-t)
    return lo,hi

def demands(a,b,t):
    need=Fraction(b+2*t)
    vals=[Fraction(x*(a+1-2*x),a-x) for x in range(a)]
    tail=[max(vals[i:]) for i in range(a)]
    cur=[]
    def rec(left,lo,score):
        if left==0:
            if score>=need:yield cur.copy()
            return
        if score+left*tail[lo]<need:return
        for x in range(lo,a):
            if score+vals[x]+(left-1)*tail[x]<need:continue
            cur.append(x);yield from rec(left-1,x,score+vals[x]);cur.pop()
    yield from rec(a,0,Fraction())

def threshold_certificate(s,b,rmax):
    """Return an exact threshold contradiction, if one exists.

    If z_h sources have residual degree >=h, residual activity gives
      r >= b+z_h(h-1),
    hence z_h <= floor((rmax-b)/(h-1)).
    The audited threshold lemma gives
      2 W_h <= z_h^2-z_h+h(h+1).
    The RHS is increasing for integer z_h>=1.
    """
    a=len(s)
    for h in range(2,a):
        high=[x for x in s if x>=h]
        if not high:continue
        zmax=min(b,(rmax-b)//(h-1))
        H=max(high);W=sum(high)
        if H>zmax:
            return {'kind':'source_count','h':h,'H':H,'zmax':zmax}
        rhs=zmax*zmax-zmax+h*(h+1)
        if 2*W>rhs:
            return {'kind':'threshold','h':h,'W':W,'zmax':zmax,'lhs':2*W,'rhs':rhs}
    return None

def dual_certificate(s,b,rmax):
    """Exact Farkas certificate for the source-capacity Hall relaxation.

    A source of residual degree j (1..a) has at most a-j selected slots and
    can select only labels with demand <=j.  For each top-k demand prefix D_k,
    A[k,j] is therefore an upper bound on that source's contribution.
    Residual activity gives sum(j-1) n_j = r-b <= rmax-b.
    """
    a=len(s);ss=sorted(s,reverse=True)
    D=[sum(ss[:k]) for k in range(1,a+1)]
    A=[[min(a-j,sum(x<=j for x in ss[:k])) for j in range(1,a+1)] for k in range(1,a+1)]
    # Floating point only proposes nonnegative prefix weights y and a free mu.
    res=linprog([-x for x in D+[b]],
                A_ub=[[A[k][j] for k in range(a)]+[1] for j in range(a)],
                b_ub=list(range(a)),bounds=[(0,None)]*a+[(None,None)],method='highs')
    if not res.success:return None
    y=[max(Fraction(),Fraction(float(x)).limit_denominator(10000)) for x in res.x[:a]]
    mu=min(Fraction(j)-sum(y[k]*A[k][j] for k in range(a)) for j in range(a))
    score=sum(y[k]*D[k] for k in range(a))+b*mu
    if score<=rmax-b:return None
    scale=lcm(mu.denominator,*(x.denominator for x in y))
    yi=[int(x*scale) for x in y];mui=int(mu*scale)
    # Exact proof event.
    assert all(mui+sum(yi[k]*A[k][j] for k in range(a))<=scale*j for j in range(a))
    assert sum(yi[k]*D[k] for k in range(a))+b*mui>scale*(rmax-b)
    return {'kind':'dual','scale':scale,'weights':yi,'mu':mui}

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--t',type=int,choices=[2,3],required=True);ap.add_argument('--output',type=Path,required=True);z=ap.parse_args()
    a=12;b=16;t=z.t;z.output.mkdir(parents=True,exist_ok=True)
    counts={};kept=[];domain=0
    for s in demands(a,b,t):
        domain+=1;lo,hi=bounds(s,b,t);rec={'s':s,'rmin':lo,'rmax':hi}
        if lo>hi:cert={'kind':'bounds'}
        else:cert=threshold_certificate(s,b,hi) or dual_certificate(s,b,hi) or {'kind':'OPEN'}
        counts[cert['kind']]=counts.get(cert['kind'],0)+1
        if cert['kind']=='OPEN':kept.append(rec)
    (z.output/'demands.json').write_text(json.dumps(kept,separators=(',',':'))+'\n')
    (z.output/'demands.txt').write_text(str(len(kept))+'\n'+''.join(' '.join(map(str,r['s']+[r['rmin'],r['rmax']]))+'\n' for r in kept))
    rep={'schema':'n29-minimal-demand-v1','scope':{'n':29,'Delta':16,'m':208+t,'a':a,'b':b,'t':t},'demand_domain':domain,'counts':counts,'retained_demands':len(kept),'support_formula_used':False,'all_nontrivial_saved_cuts_exactly_checked':True}
    (z.output/'MINIMAL_PREPARE_REPORT.json').write_text(json.dumps(rep,indent=2)+'\n');print(json.dumps(rep))
if __name__=='__main__':main()
