// Minimal audited residual-row scanner for n=29, Delta=16.
// No project dependencies.  The only pruning rules are source-capacity Hall
// bounds and the monotone supplement-cap refinement proved in the hostile audit.
#include <array>
#include <vector>
#include <fstream>
#include <iostream>
...
int main(int argc,char**argv){
 if(argc!=5){cerr<<"minimal_rows demands.txt survivors.txt bands.txt t\n";return 2;}
 int t=stoi(argv[4]);if(t!=2&&t!=3)return 2;
 ...
 for(int id=0;id<N;id++){
  ...
  for(int total=lo;total<=hi;total++)for(const Row&rho:bucket[total]){
   ...
   // Actual q_u <= 12-rho_u, and selected labels at u must satisfy s_i<=rho_u.
   for(int u=0;u<16;u++){
    int elig=0;for(int x:s)elig+=(x<=rho[u]);
    cap[u]=min(12-rho[u],elig);
   }
   auto violates=[&](){
    // Hall on every largest-demand prefix.  A source contributes at most its
    // q-cap and at most one incidence to each eligible label.
    for(int k=1;k<=12;k++){
   ...
   // If source u has actual selected degree q, its q distinct supplements w
   // all satisfy rho_w+q_w>=q-1.  Replacing q_w by a valid upper cap gives a
   // monotone necessary upper-cap refinement.
   for(;;){
    for(int u=0;u<16;u++){
     int best=0;
