...
class Model:
    ...
    def equal(self,d,b=0):self.eq.append((dict((j,c) for j,c in d.items() if c),b))
    def le(self,d,b):self.ub.append((dict((j,c) for j,c in d.items() if c),b))
    def arrays(self):
        def mat(rows):
            rr=[];cc=[];vv=[]
    ...
    def solve(self):
        A,b,E,f=self.arrays();return linprog(np.zeros(len(self.names)),A_ub=A,b_ub=b,A_eq=E,b_eq=f,bounds=(0,None),method='highs')
...
def build(s,rho,t):
    ...
    # Label (d,R) options. T_h is the unnormalised tail Pr[x>=h] for the
    # actual selected column degree x. This avoids inheriting endpoint-x types.
    ...
    # Actual source selected/supplement degrees q,p.
    for k,(rh,nk) in enumerate(SG):
        ...
        m.equal(norm,1)
    # Independent q-flow for selected source->supplement pairs. Compatibility
    # is exactly rho_w+q_w >= q_u-1; each unordered B-pair has capacity one.
    P={};P_out=defaultdict(list);P_in=defaultdict(list);P_pair=defaultdict(list)
        ...
        for l,(rhl,nl) in enumerate(SG):
            ...
            for q in qks:
                for q2 in qls:
                    ...
                    z=m.var(('P',k,l,q,q2));P[k,l,q,q2]=z
    ...
    for (k,l),items in P_pair.items():
        if k==l and SG[k][1]<2:continue
        e={}
        for kk,ll,z in items:add(e,z,1)
        m.le(e,1)
    # Selected incidence flow between actual source qp types and label (d,R)
    # options. The endpoint load R+x >= q+p is encoded through nested x tails.
    Z={};by_source=defaultdict(list);by_label=defaultdict(list)
    for k,(rh,nk) in enumerate(SG):
        for g,(sg,ng) in enumerate(LG):
            for q,p,w in source_types[k]:
                ...
                for d,R,y,U in label_opts[g]:
                    ...
                    z=m.var(('Z',k,g,q,p,d,R));Z[k,g,q,p,d,R]=z
    ...
    for g,(sg,ng) in enumerate(LG):
        for d,R,y,U in label_opts[g]:
            items=by_label[g,d,R];total={}
            for k,q,p,z in items:add(total,z,SG[k][1])
            e=dict(total)
            for h in range(1,U+1):add(e,T[g,d,R,h],-ng)
            m.equal(e,0)
            # Nested Hall capacity: selected incidences requiring x>=h can use
            # only labels with x>=h. Their total selected-slot capacity is
            # E[x 1{x>=h}] = (h-1)T_h + sum_{j=h}^U T_j.
            for h in range(1,U+1):
                e={}
                for k,q,p,z in items:
                    if max(1,q+p-R)>=h:add(e,z,SG[k][1])
                add(e,T[g,d,R,h],-ng*(h-1))
                for hh in range(h,U+1):add(e,T[g,d,R,hh],-ng)
                m.le(e,0)
...
def exact_certificate(m):
    # Floating point only proposes a Farkas ray. Acceptance is exact integer
    # arithmetic below and in verify_certificate.
    A,b,E,f=m.arrays();N=len(m.names);nu=len(m.ub);ne=len(m.eq)
