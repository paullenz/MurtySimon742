...
def main():
    ...
    demands=json.loads((a.root/'demands.json').read_text());rows=json.loads((a.root/'projected_survivors.json').read_text());positions=[i for i in range(len(rows)) if i%a.shards==a.shard];start=time.time()
