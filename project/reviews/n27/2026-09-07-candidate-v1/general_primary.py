...
def matches(easiest_labels: list, strongest_suppliers: list) -> bool:
    return len(easiest_labels) <= len(strongest_suppliers) and all(d <= c for d, c in zip(reversed(easiest_labels), strongest_suppliers))
...
def run(output: Path, degree_a: int, original_edges: int, ks: list[int], order: int = 27) -> dict:
    ...
    summary = dict(schema=1, implementation='primary',
        scope=dict(n=order,edges_G=original_edges,delta_G=B,degree_A=A,gap=GAP,ledger=LEDGER,ks=ks),
...
if __name__ == '__main__':
    ...
    args = p.parse_args()
    try:
        print(json.dumps(run(args.output,args.a,args.edges,args.ks,args.n), sort_keys=True))
