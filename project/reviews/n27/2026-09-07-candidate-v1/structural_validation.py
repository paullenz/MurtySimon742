...
n=27. Small representatives use every base vertex and every selected-edge
...
def extended_checks(A,B,ds,rho,R,q,selected):
    # Actual graph validation of the two new necessary inequalities.
    for j in range(1,max(ds.values())+1):
        demand=sum(max(0,ds[i]-R[i]) for i in A if ds[i]>=j)
        capacity=sum(rho[u]+rho[v]>=j for u,v in combinations(B,2))
        ensure(demand<=capacity,'fixed_column_pair_threshold')
    for variant in range(3):
        caps={b:q[b] if variant==0 else min(len(A)-rho[b],q[b]+1) if variant==1 else len(A)-rho[b] for b in B}
        demand={i:max(0,ds[i]-R[i]) for i in A}
        eligible={i:{b for b in B if ds[i]<=rho[b]+caps[b]-1 and ds[i]<=rho[b]+R[i]} for i in A}
        ensure(all(demand[i]<=len(eligible[i]) for i in A),'new_label_capacity')
        forced={b:sum(b in eligible[i] and demand[i]==len(eligible[i]) and demand[i]>0 for i in A) for b in B}
        ensure(all(forced[b]<=q[b] for b in B),'forced_source_lower_bound')
        pairs=set()
        for i in A:
            for b in eligible[i]:
                lower=max(1,forced[b],ds[i]-rho[b]+1)
                for w in B:
                    if w!=b and ds[i]<=rho[b]+rho[w] and lower<=caps[b] and rho[w]+caps[w]>=lower-1:
                        pairs.add(tuple(sorted((b,w))))
        ensure(all(tuple(sorted((b,w))) in pairs for (i,b),w in selected.items()),'selected_pairs_retained_by_new_filter')
        ensure(sum(demand.values())<=len(pairs),'supplement_pair_budget')
    ...
    for v in bases:
        ...
        for choice in assignments:
            ...
            if is_min:
                ...
                extended_checks(A,B,ds,rho,R,q,selected)
...
def main():
    ...
    generated=[]
    for n in [7,8,9,10,12,14,16,20,25,27]:
