...
def run(output: Path, reference: Path | None, degree_a: int, original_edges: int, ks: list[int]) -> dict:
    ...
    summary['scope'] = dict(n=25, edges_G=original_edges, degree_A=A, delta_G=B, gap=GAP, ledger=LEDGER, ks=ks)
...
if __name__ == '__main__':
    ...
    a = p.parse_args()
    try:
        print(json.dumps(run(a.output, a.reference, a.a, a.edges, a.ks), sort_keys=True))
