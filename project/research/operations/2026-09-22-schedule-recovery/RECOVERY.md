...
The research task was enabled and returned a last-run timestamp of 22 September 08:56:15 BST. This is not proof of useful execution. The automation interface exposes no detailed run-history/error retrieval. An immediate run was requested successfully on the predecessor during recovery but no STARTED checkpoint appeared during subsequent checks. The predecessor has now been disabled and replaced in this fresh recovery conversation. A manual recovery request on the replacement was accepted after publication of commit `448831064349ffd104c473f4877f525862e367eb`; the clock observed immediately before the request was 09:13:12 BST. No completion is claimed. See `RUN_REQUEST.json`. Disabling a schedule does not prove that a previously queued invocation was cancelled. Duplicate-writer safeguards remain necessary. The browser was not signed in, so no run history was read through that route.
