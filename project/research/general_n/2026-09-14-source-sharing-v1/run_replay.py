...
def main():
    ...
    for name, expected in HASHES.items():
        ...
        if actual != expected:
            raise ValueError(f'Frozen source/evidence hash mismatch: {name}: {actual}')
    ...
    for program, expected_file in [('verify_source_prices.py', 'VERIFICATION.json'),
                                   ('source_prices.py', 'ORIGINAL_SIX_RESULTS.json')]:
        process = subprocess.run([sys.executable, str(HERE / program)],
    ...
    print('PASS: pinned sources, canonical input, copied six-row arrays, finite soundness and 576 exact price evaluations.')
