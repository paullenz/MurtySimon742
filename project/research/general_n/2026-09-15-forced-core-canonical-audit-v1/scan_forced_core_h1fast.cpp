...
// Exact symmetric-bin relaxation for forced-core receivers. Each candidate receiver
// may be unused or dedicated to one core label. If used, its full favourable
// effective capacity min(c_v, |U|) is credited to that label. The returned cost
// is the minimum unavoidable high-slot loss among all feasible assignments.
static pair<bool,int> core_partition(const vector<int>&caps,const vector<int>&loss,int h,int m){
    ...
    if(h==1){
        const int INF=INT_MAX/4;vector<int>dp(m+1,INF);dp[0]=0;
        for(size_t z=0;z<caps.size();++z){int cap=min(caps[z],m),w=loss.empty()?0:loss[z];vector<int>nd=dp;for(int x=0;x<=m;++x)if(dp[x]<INF){int y=min(m,x+cap);nd[y]=min(nd[y],dp[x]+w);}dp.swap(nd);}
        return dp[m]>=INF?make_pair(false,INT_MAX):make_pair(true,dp[m]);
    }
