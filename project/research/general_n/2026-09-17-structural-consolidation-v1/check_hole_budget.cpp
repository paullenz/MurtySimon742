// Bounded-hole (h=2) checker: adjacency matrix and explicit paths of length <=2.
...
int main() {
    ...
    while(std::cin >> id >> d >> q >> cm >> xm >> cx) {
        ...
        bool support=(xm==0 || d-xs<=2 || (xs==1 && cx==1));
        bool eligible=a[0][1] && deg0>=3 && deg1>=3 && cm==full && support;
        for(int i=0;i<d;++i) {
            a[i][d]=a[d][i]=(cm>>i)&1;
