#!/usr/bin/env python3
"""Parallel exact physical-source screen for r=11 support-seven strict cores."""
import itertools,json,multiprocessing as mp
from functools import lru_cache
pairs=list(itertools.combinations(range(7),2))
def one(line):
 f=list(map(int,line.split()));case,mask,t=f[:3];R=tuple(f[3:10]);k=7
 edges=[pairs[q] for q in range(21) if mask>>q&1];neigh=[set() for _ in R];deg=[0]*k
...
if __name__=='__main__':
 lines=open('r11_support7_masks.txt').read().splitlines()
 with mp.Pool(6) as pool:rows=[r for r in pool.imap_unordered(one,lines) if r is not None]
 counts={str(i):sum(r['case']==i for r in rows) for i in range(5)}
