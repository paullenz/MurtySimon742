#!/usr/bin/env python3
"""Exact early-exit supplement search for 8 r=11 support-nine kernels."""
import itertools
import json
import multiprocessing as mp
import time

ROWS = json.load(open("r11_support9_sources_case1.json"))["feasible_rows"]
...
def solve(item):
    ...
    return {"index": index, "mask": row["mask"], "R": residual,
...
if __name__ == "__main__":
    ...
    print(json.dumps({
        ...
        "rows": rows,
        "scope": "Exact DFS: EXHAUSTED rules out all abstract source multisets under inherited one-supplement condition; not graph realization.",
    }, indent=2))
