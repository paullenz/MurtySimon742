...
The script proves by exact finite algebra that the A1 certificate gap equals
  1 + 1454/135 (U-36) + 125/54 (24-V) + 23/9 (Z-16),
where U=sum_{s_i>=3}s_i, V=sum_{s_i>=4}s_i, Z=sum_{rho_u>=4}rho_u.
It then verifies the scalar supplement-cutoff/total-incidence argument that
forces V<=24 (equivalently D4<=6) throughout this support class. Together with
h=4 and zero slack, this forces U>=36 and Z>=16, hence gap>=1.
...
def a1_envelopes(mod):
 ...
 for s in EXPECTED_ELL:
  vals=[lam*R+c*x+x*mod.F(R+s,B-R-x) for R in range(10-s+1) for x in range(s,B-R+1)]
  ell[s]=min(vals)
 sig={}
 # qmax is support-dependent. In this regime: qmax(1)=0, qmax(3)>=?; the
 # minima below are stable for every qmax occurring in the abstract domain.
 for r in EXPECTED_SIG:
  qms=range(0,A-r+1)
  stable=[]
  for qm in qms:
   ...
   stable.append((qm,min(vals)))
  # Actual qmax ranges: r=1 ->0; r=3 depends on D3; r>=4 ->A-r.
  if r==1:sig[r]=dict(stable)[0]
  elif r==3:
   # D3=12-D4 and D4 ranges 4..12 in the abstract h/zero-slack domain;
   # only qmax values actually realized below are checked later profilewise.
   sig[r]=None
  else:sig[r]=dict(stable)[A-r]
 if ell!=EXPECTED_ELL:raise AssertionError(('ell',ell))
 for r in (1,4,5,6):
  if sig[r]!=EXPECTED_SIG[r]:raise AssertionError(('sig',r,sig[r]))
 return th,ell
...
def gap_direct(mod,p):
 return mod.gaps_for([p],mod.TEMPLATES['A1'])[0]
def tails(p):
...
def main():
 ...
 mod=load_module(z.cover_script);th,ell=a1_envelopes(mod)
 D=enumerate_domain()
 ...
 for p in D:
  ...
  # Verify the claimed source envelope at every actually occurring qmax(3).
  ...
  C,L,qstar=scalar_cutoff(p);Qcap=sum(qstar)
  feasible_total=p['S']<=Qcap
  rec={'d':list(p['d']),'r':list(p['r']),'S':p['S'],'R':p['R'],'L':L,'total_cap':Qcap,'U':tails(p)[0],'V':tails(p)[1],'Z':tails(p)[2],'gap':rat(gd)}
  (survivors if feasible_total else rejected).append(rec)
 ...
 abstract={(tuple(x['d']),tuple(x['r'])) for x in survivors}
 actual=set()
 for i,sc,rc,g in cell:
  actual.add(((sc[2],sc[3],sc[4]),(rc[3],rc[4],rc[5],rc[6])))
 ...
 out={'schema':'n29-t3-a1-slack-identity-exact-v1','status':'PASS','scope':{'n':29,'Delta':16,'t':3,'h':4,'J':14,'support_s':[2,3,4],'support_rho':[1,3,4,5,6],'rho1':9},'abstract_histogram_domain':121,'total_capacity_survivors':11,'total_capacity_rejections':110,'survivors_exactly_D4_le_6':True,'frontier_cell_profiles':11,'frontier_cell_equals_abstract_survivors':True,'a1_envelopes':{'ell':{str(k):rat(v) for k,v in EXPECTED_ELL.items()},'sigma':{str(k):rat(v) for k,v in EXPECTED_SIG.items()},'lambda':rat(th['lambda'])},'gap_identity':'gap = 1 + (1454/135)(U-36) + (125/54)(24-V) + (23/9)(Z-16)','tails':{'U':'sum s_i over s_i>=3','V':'sum s_i over s_i>=4','Z':'sum rho_u over rho_u>=4'},'survivor_slacks':{'U_min':min(x['U'] for x in survivors),'V_max':max(x['V'] for x in survivors),'Z_min':min(x['Z'] for x in survivors)},'minimum_gap':[1,1],'unique_equality_histogram':eq[0],'frontier_unique_equality_profile':1,'scalar_cutoff_argument':{'terminal_cap':'q*_u=min(C_u,L)','C_u':'min(12-rho_u, #{i:s_i<=rho_u})','necessary_total_incidence':'sum_i s_i <= sum_u q*_u'},'qmax3_values_checked':sorted(qmax3_values),'arithmetic':'fractions.Fraction exact rational','solver_used':False,'floating_point_used':False,'interpretation':'Within the audited A1 support class, the supplement scalar cutoff plus total selected-incidence capacity alone forces D4<=6. h=4 and zero slack then force U>=36 and Z>=16, so the exact A1 gap identity gives gap>=1 with unique equality at frontier profile 1. Universal progress now reduces to deriving the support class (or an equivalent inequality) without finite frontier preparation.'}
