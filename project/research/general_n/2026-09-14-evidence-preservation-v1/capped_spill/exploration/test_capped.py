...
for j,raw in enumerate(rows):
 ...
 E=p['Esel'];env=m.envelope(s,q,rho,p['b']-p['a'])
...
print(C)
print('new rows',[x['row'] for x in newex]);print('new sample',json.dumps(newex[:1],indent=2))
print('unrejected sample',json.dumps(survivors[:1],indent=2))
Path('/mnt/data/work/spill/capped_results.json').write_text(json.dumps(dict(counts=dict(C),newly_rejected=newex,survivors=survivors),indent=2))
