...
int main(){
 ...
 for(auto g:reps){
  ...
  for(int s=0;s<512;s++){
   if(popcount((unsigned)s)>5||seen.count(s))continue;
   ...
   bool P[10],valid=true;P[0]=degree[0]>3;if(P[0]&&(degree[0]-3)*(degree[0]-3)>degree[0])valid=false;
   for(int i=0;i<9;i++){P[i+1]=degree[i+1]>1;int adjR=internal[i]+(((s>>i)&1)?3:0);if(P[i+1]&&(degree[i+1]-1)*(degree[i+1]-1)>adjR)valid=false;}
   if(!valid)continue;
   int eP=0,eN=0;for(int i=0;i<9;i++)for(int j=i+1;j<9;j++)if(a[i][j]){if(P[i+1]&&P[j+1])eP++;if(!P[i+1]&&!P[j+1])eN++;}
   for(int i=0;i<9;i++)if(s>>i&1){if(P[0]&&P[i+1])eP++;if(!P[0]&&!P[i+1])eN++;}
   int sumP=P[0]?3:0,slack=0;for(int i=0;i<9;i++){if(P[i+1])sumP++;else if(degree[i+1]==0)slack++;}
   int t=eP-eN-sumP-slack;if(t<=0)continue;
   strict++;cout<<g<<" "<<s<<" "<<t;cout<<" 3";for(int i=0;i<9;i++)cout<<" 1";for(int i=0;i<10;i++)cout<<" "<<degree[i];cout<<"\n";
  }
 }
