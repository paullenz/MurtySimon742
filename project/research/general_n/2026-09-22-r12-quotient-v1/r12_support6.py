#!/usr/bin/env python3
"""Exact r=12 quotient/source screen for residual support exactly six."""
...
for R in parts(12):
    if len(R)!=6:continue
    pairs=list(itertools.combinations(range(6),2));idx={e:q for q,e in enumerate(pairs)}
    perms=list(perms_for(R));reps={}
    for mask in range(1<<15):
        d=datum(R,pairs,mask)
        if d is None:continue
        best=min(sum(1<<idx[tuple(sorted((p[i],p[j])))]
                     for q,(i,j) in enumerate(pairs) if mask>>q&1) for p in perms)
        if best not in reps:reps[best]=datum(R,pairs,best)
    feasible=[]
    for mask,d in reps.items():
        w=source(R,d)
        if w is not None:feasible.append({'mask':mask,'t_upper':d[4],'edges':d[0],
                                          'degrees':d[1],'witness':w})
    out[str(R)]={'strict_orbits':len(reps),'source_feasible':len(feasible),'feasible':feasible}
    print(R,len(reps),len(feasible),file=sys.stderr,flush=True)
