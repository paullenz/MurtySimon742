#!/usr/bin/env python3
"""Parallel exact physical-source screen for r=12 support-seven strict cores."""
...
if __name__=='__main__':
 lines=open('r12_support7_masks.txt').read().splitlines()
 with mp.Pool(8) as pool:rows=[r for r in pool.imap_unordered(one,lines,chunksize=2) if r is not None]
 counts={str(i):sum(r['case']==i for r in rows) for i in range(7)}
