...
def read_bytes(name):
    ...
    obj=json.loads(original) if item['format']=='json' else None
    count=len(original.splitlines()) if item['format']=='jsonl' else len(obj[item['records_key']]) if 'records_key' in item else len(obj)
