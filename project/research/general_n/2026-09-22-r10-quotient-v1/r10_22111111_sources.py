#!/usr/bin/env python3
"""Parallel exact source feasibility for r=10 partition (2,2,1^6)."""
import itertools,json,multiprocessing as mp
from functools import lru_cache
R=(2,2,1,1,1,1,1,1);k=8;pairs=list(itertools.combinations(range(k),2))
def one(line):
 f=list(map(int,line.split()));mask,t=f[:2];edges=[pairs[q] for q in range(28) if mask>>q&1]
 ...
 def solve(rem,x):
  ...
  return None
 w=solve(R,(0,)*k)
 return None if w is None else {'mask':mask,'t_upper':t,'edges':edges,'degrees':deg,'P':sorted(P),'need':need,
  'patterns':len(patterns),'states':solve.cache_info().currsize,'witness':[patterns[q][2] for q in w]}
if __name__=='__main__':
 lines=open('r10_22111111_masks.txt').read().splitlines()
 with mp.Pool(6) as pool: rows=[r for r in pool.imap_unordered(one,lines) if r is not None]
 print(json.dumps({'R':R,'core_orbits':len(lines),'source_feasible':len(rows),'rows':sorted(rows,key=lambda x:x['mask']),
