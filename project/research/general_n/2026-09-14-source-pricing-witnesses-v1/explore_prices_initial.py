...
SCALE=100
rows=json.load(open('/mnt/data/ms742/inputs.json'))['rows']
...
for mode in [0,1]:
 for xi in sorted(set([0,min(v for v in row['s'] if v>0)])):
  ...
  for iteration in range(45):
   ...
   seen.add(cut)
   constraints.append(gradient+[-1]);rhs.append(-base)
   res=linprog([0]*b+[1],A_ub=constraints,b_ub=rhs,
               bounds=[(-100*SCALE,100*SCALE)]*b+[(None,None)],method='highs')
   if not res.success: break
   price=[int(round(x)) for x in res.x[:b]]
...
json.dump({'scope':'original six retained profiles; finite price search only','results':results},
          open('/mnt/data/ms742/prices_'+str(row['row'])+'.json','w'),indent=2)
