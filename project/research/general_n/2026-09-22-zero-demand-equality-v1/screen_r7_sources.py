#!/usr/bin/env python3
"""Necessary physical-source screen for r=7 strict-surplus core orbits."""
import itertools,json
from pathlib import Path
data=json.loads(Path('R7_CORE_SCREEN.json').read_text());out={}
for partition,entry in data.items():
    if not entry.get('orbit_representatives'):continue
    ...
    for q,orbit in enumerate(entry['orbit_representatives'],1):
        ...
        P=set(rep['P']);need=tuple(rep['selected_lower_bounds']);patterns=[];free=0
        for state in itertools.product(range(3),repeat=k):
            ...
            if not Z:
                assert not (L&P);free+=bool(L);continue
        ...
        def search(start,remaining,x,chosen):
            if not any(remaining):
                if all(x[i]>=need[i] for i in range(k)):
                    count[0]+=1
                    if len(examples)<3:examples.append(chosen)
                return
        ...
        out[f'{partition}/orbit_{q}']={'R':R,'edges':edges,'P':sorted(P),
          't_upper':rep['t_upper'],'selected_lower_bounds':need,
          'single_source_patterns':len(patterns),'residual_free_N_patterns_omitted':free,
          'surviving_source_multisets':count[0],'first_survivors':examples,
          'scope':'Necessary C-source states only; no supplements, B-edges, zero attachments, criticality or graph realization.'}
