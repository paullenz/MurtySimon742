#!/usr/bin/env python3
"""RX4 Hall core with exact grouped source/supplement transport restored.

Keeps W and P source-transport variables, label extra-load distributions and
two-rectangle Hall cuts. Still deletes residual R variables/budget and selected
incidence Z variables, cumulative tails and unordered-pair aggregate capacity.
This isolates whether exact source/supplement transport is the missing t=1
ingredient after RX4.
"""
...
def build(a,b,dmax,s,rho):
    ...
    for D in domains:
        ...
        m.le(row,0)
    ex.add_unit_density_bounds(m); m.meta={'source_attribute_pairs':len(attrs),'one_or_two_rectangle_domains':len(domains),'transport_variables':sum(len(v) for v in Pout.values())}; return m
...
def main():
    ...
    for hp,(pos,did,total,s,rho) in enumerate(H):
        M=build(a.a,a.b,a.dmax,s,rho); cert=ex.exact_certificate(M); dc.append(M.meta['one_or_two_rectangle_domains']); z={'hard_position':hp,'position':pos,'demand_id':did,'total':total,'s':s,'rho':rho,'metadata':M.meta,'certificate':cert}; rec.append(z)
    ...
    R={'schema':'general-rx-hall-rx4-exact-transport-core-v1','scope':{'a':a.a,'b':a.b,'dmax':a.dmax,'t':a.t},'hard_rows':len(H),'exact_integer_farkas_rejections':len(H)-len(unr),'unresolved':unr,'hall_domains_min':min(dc) if dc else None,'hall_domains_max':max(dc) if dc else None,'rhs_min':min(rhs) if rhs else None,'rhs_max':max(rhs) if rhs else None,'uses_exact_grouped_transport':True,'uses_rx4':True,'removes_R_residual_variables_and_budget':True,'removes_Z_incidence_variables':True,'uses_two_rectangle_hall':True,'uses_unordered_pair_aggregate_capacity':False,'uses_cumulative_tail_variables':False,'floating_point_is_proposal_only':True,'integer_farkas_is_acceptance':True,'json_sha256':hashlib.sha256(raw).hexdigest(),'gzip_sha256':hashlib.sha256(gz).hexdigest()}
    ...
    if unr: raise SystemExit(f'{len(unr)} exact-transport core rows lack exact certificates')
