...
point is separated by max-flow/min-cut on the ABC compatibility graph. Each
violated Hall cut has integer coefficients and is appended to the master.
Final infeasibility is accepted only through the existing exact integer-Farkas
checker. The final report also identifies which generated Hall rows receive
nonzero multipliers in the exact certificate.
"""
...
def separate(sol,sources,labels):
    activeL=[v for v in labels if v['n']*v['x']*sol[v['var']]>1e-9];activeS=[u for u in sources if u['n']*u['q']*sol[u['var']]>1e-9]
    nl,ns=len(activeL),len(activeS);S=0;lo=1;so=lo+nl;T=so+ns;D=Dinic(T+1);total=0.0
    ...
    if lhs<=1e-7:raise RuntimeError(f'mincut failed to yield violated Hall row: {lhs}')
    sig=(tuple(sorted((v['g'],v['R'],v['x']) for v in U)),tuple(sorted((u['k'],u['q'],u['p']) for u in N)))
    return row,lhs,sig,U,N
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--demands-json',type=Path,required=True);p.add_argument('--rows',type=Path,required=True);p.add_argument('--a',type=int,required=True);p.add_argument('--b',type=int,required=True);p.add_argument('--dmax',type=int,required=True);p.add_argument('--t',type=int,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--max-cuts',type=int,default=1000);a=p.parse_args()
    records=[]
    for hp,(did,total,s,rho) in enumerate(load(a.demands_json,a.rows,a.t)):
        m,sources,labels=build_master(a.a,a.b,a.dmax,s,rho);cuts=[];sigs=set();numerical_status=None;hall_start=len(m.ub)
        ...
        hall_end=len(m.ub);ex.add_unit_density_bounds(m);cert=ex.exact_certificate(m)
        active=[]
        if cert is not None:
            for i,w in cert['ub']:
                if hall_start<=i<hall_end:active.append({'cut_index':i-hall_start,'weight':w})
        records.append({'hard_position':hp,'demand_id':did,'s':s,'rho':rho,'hall_cuts_added':len(cuts),'proof_active_hall_cuts':active,'proof_active_hall_count':len(active),'cuts':cuts,'numerical_status_before_exact':numerical_status,'rejected_exactly':cert is not None,'certificate_rhs':None if cert is None else cert['rhs']})
    out={'schema':'rz-hall-cut-generation-v2','scope':{'a':a.a,'b':a.b,'dmax':a.dmax,'t':a.t},'hard_rows':len(records),'exact_rejections':sum(r['rejected_exactly'] for r in records),'survivors':sum(not r['rejected_exactly'] for r in records),'cut_counts':[r['hall_cuts_added'] for r in records],'proof_active_cut_counts':[r['proof_active_hall_count'] for r in records],'records':records,'interpretation':'Z-free exact Hall projection. Numerical max-flow/LP proposes integer Hall rows; final rejection requires independently checked integer Farkas certificate. proof_active_hall_cuts are the generated Hall rows with nonzero exact Farkas weight.'}
    a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(out,indent=2,sort_keys=True)+'\n');print(json.dumps({'exact_rejections':out['exact_rejections'],'survivors':out['survivors'],'cut_counts':out['cut_counts'],'proof_active_cut_counts':out['proof_active_cut_counts']},indent=2))
