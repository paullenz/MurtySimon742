...
def main():
    ...
    B=json.loads(z.boundary_json.read_text());S=json.loads(z.support_json.read_text());C=json.loads(z.correction_json.read_text());raw=B['trimmed_survivor_records'];P=[{'s':r['s'],'rho':r['rho'],'demand_id':int(r['demand_id']),'hard_position':int(r['hard_position'])} for r in raw]
    ...
    for name,f in funcs.items():
        ...
        for key,G in sorted(groups.items()):
            ...
            active_bc=int(sum(bool(res.success and res.x[M.idx[('BC',i)]]>1e-8) for i in range(len(bc)))) if res.x is not None else None
