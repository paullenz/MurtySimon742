...
For each requested n30 t=1 hard profile separately, solve one LP containing all
996 n29 profiles plus that single n30 profile, with the exactified n29 9 BC + 7
SH support and one common staircase-weight vector. Scalar dual bookkeeping stays
profile-specific. Floating reconnaissance only. Use --only-index to shard the
seven tests without changing their mathematics.
"""
...
def main():
    ...
    items=list(enumerate(N30['records']))
    if z.only_index is not None:
        items=[x for x in items if x[0]==z.only_index]
        if not items:raise SystemExit(f'n30 index {z.only_index} not found')
    rec=[]
    for i,r in items:
    ...
    out={'schema':'cross-order-n30-single-obstruction-scan-v2','n29_profiles':len(P29),'n30_profiles_tested':len(rec),'requested_index':z.only_index,'support_count':len(bc)+len(sh),'BC_count':len(bc),'SH_count':len(sh),'feasible_indices':[r['n30_index'] for r in rec if r['success']],'infeasible_indices':[r['n30_index'] for r in rec if not r['success']],'floating_point_reconnaissance_only':True,'records':rec,'interpretation':'Each test shares one staircase-weight vector across all 996 n29 profiles and exactly one n30 hard profile.'}
