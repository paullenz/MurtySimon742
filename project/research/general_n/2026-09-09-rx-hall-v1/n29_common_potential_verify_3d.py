#!/usr/bin/env python3
"""Independent standard-library exact replay for n=29 RX-Hall 3-D potentials.

Rebuilds variable order and every label/source envelope plus profile-margin
inequality without importing NumPy, SciPy, the LP builder, or the exactifier.
Supports arbitrary listed BC layers, diagonal K thresholds, and one first-
coordinate SH threshold S.  Intended for exact artifacts whose acceptance has
already been reduced to integer numerators over a common positive scale.
"""
...
def main():
 ...
 layers=tuple(E['layers']);diagKs=tuple(E.get('diagonal_K',[]));S=int(E['SH_threshold']);rect_support={tuple(x) for x in E['rectangle_support']};scale=int(E['scale']);X=list(map(int,E['integer_numerators']));names=[];rect=[]
 for D in layers:
  for V in range(17):rect.append((D,V,alloc(names,('BCrect',D,V))))
 diag=[(K,alloc(names,('J',K))) for K in diagKs];sh=alloc(names,('SH',S));meta=[]
 ...
 def pot(d,h,scoord,factor,sign):
  v=16-h;tot=0
  for D,V,w in rect:
   if d>=D and v>=V:tot+=sign*factor*X[w]
  for K,w in diag:
   if d+v>=K:tot+=sign*factor*X[w]
  if scoord>=S:tot+=sign*factor*X[sh]
  return tot
 if length_ok:
  for D,V,w in rect:
  ...
  for K,w in diag:
   if X[w]<0:bviol.append((str(names[w]),'negative',X[w]))
  if X[sh]<0:bviol.append((str(names[sh]),'negative',X[sh]))
  fixed=E.get('fixed_integer_weights',{})
  for D,V,w in rect:
   key=f'B({D},{V})'
   if key in fixed and X[w]!=int(fixed[key])*scale:bviol.append((key,'fixed_weight',X[w],int(fixed[key])*scale))
  for K,w in diag:
   key=f'J({16-K})'
   if key in fixed and X[w]!=int(fixed[key])*scale:bviol.append((key,'fixed_weight',X[w],int(fixed[key])*scale))
  if 'SH'+str(S) in fixed and X[sh]!=int(fixed['SH'+str(S)])*scale:bviol.append((f'SH{S}','fixed_weight',X[sh],int(fixed['SH'+str(S)])*scale))
  for pi,pf in enumerate(P):
   ...
   for s in sorted(sc):
    ...
    for R in range(10-s+1):
     for x in range(s,16-R+1):
      lhs=X[e]-R*X[lam]-x*X[cc]+pot(R+s,R+x,s,x,-1);zero_max=lhs if zero_max is None else max(zero_max,lhs)
   ...
   for rho in sorted(rc):
    ...
    for q in range(qmax+1):
     ...
     for pp in range(pmax+1):
      ...
      lhs+=pot(rho+q-1,q+pp,rho,q,1);zero_max=lhs if zero_max is None else max(zero_max,lhs)
 ...
 out={'schema':'n29-common-potential-independent-3d-replay-v1','t':z.t,'status':'PASS' if ok else 'FAIL','profiles':len(P),'variables_expected':len(names),'variables_received':len(X),'name_order_hash_ok':hash_ok,'scale':scale,'SH_threshold':S,'row_violation_count':len(viol),'row_violations':viol,'bound_violation_count':len(bviol),'bound_violations':bviol[:20],'zero_rhs_max_lhs':zero_max,'margin_count':len(marg),'worst_margin_numerator':max(marg) if marg else None,'required_margin_numerator':-scale,'standard_library_only':True,'imports_lp_builder':False,'imports_exactifier':False}
