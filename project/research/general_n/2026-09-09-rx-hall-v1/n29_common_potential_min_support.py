#!/usr/bin/env python3
"""Minimum-support MILP for one common n29 F+G potential across 38 profiles.

Candidate dictionary is the full generated 75 BC + 12 SH pool from the exact
pairwise correction run (including the specialised n30 base). Staircase weights
are shared across profiles; profile-specific scalar threshold/residual duals are
allowed exactly as in n29_common_potential_profile_scalars. Binary selectors
minimise the number of staircase shapes. Floating/MILP reconnaissance only.
"""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--boundary-json',type=Path,required=True);ap.add_argument('--support-json',type=Path,required=True);ap.add_argument('--correction-json',type=Path,required=True);ap.add_argument('--a',type=int,required=True);ap.add_argument('--b',type=int,required=True);ap.add_argument('--dmax',type=int,required=True);ap.add_argument('--big-m',type=float,default=200);ap.add_argument('--time-limit',type=float,default=600);ap.add_argument('--output',type=Path,required=True);z=ap.parse_args();B=json.loads(z.boundary_json.read_text());S=json.loads(z.support_json.read_text());C=json.loads(z.correction_json.read_text());P=[{'s':r['s'],'rho':r['rho'],'demand_id':r['demand_id']} for r in B['trimmed_survivor_records']]
    ...
    for fam,count in [('BC',len(bc)),('SH',len(sh))]:
        for i in range(count):
            y=len(names);names.append(('Y',fam,i));lb.append(0);ub.append(1);integ.append(1);cost.append(1.0);selectors.append((fam,i,y));w=base.idx[(fam,i)];rows.append({w:1,y:-z.big_m});los.append(-np.inf);his.append(0)
