#!/usr/bin/env python3
"""Minimum-support MILP inside the 25-shape L1-active n29 common library.

Staircase weights are common across all 38 profiles; profile-specific scalar
multipliers/envelopes are retained. Floating/MILP reconnaissance only.
"""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--boundary-json',type=Path,required=True);ap.add_argument('--common-json',type=Path,required=True);ap.add_argument('--a',type=int,required=True);ap.add_argument('--b',type=int,required=True);ap.add_argument('--dmax',type=int,required=True);ap.add_argument('--big-m',type=float,default=200);ap.add_argument('--time-limit',type=float,default=300);ap.add_argument('--output',type=Path,required=True);z=ap.parse_args();B=json.loads(z.boundary_json.read_text());J=json.loads(z.common_json.read_text());P=[{'s':r['s'],'rho':r['rho'],'demand_id':r['demand_id']} for r in B['trimmed_survivor_records']];q=J['modes']['full'];assert q['success'];bc=[tuple(tuple(p) for p in g) for g in q['active_BC_shapes']];sh=[tuple(tuple(p) for p in g) for g in q['active_SH_shapes']]
