...
The single-second-star closure is likewise UNSAT for every balanced parity population `r=q=2,...,8` on the fixed eight-star support. Extra parity multiplicity repairs neither minimal closure; any surviving split requires a genuinely distributed extra-common-neighbour system or larger star multiplicity.

Both simple closures remain UNSAT when every star-centre multiplicity is increased uniformly from two through eight at `r=q=2`. More decisively, a direct encoding forces one star to be the **unique** common A-neighbour of two vertex-disjoint missing parity pairs while leaving both cross pairs and every other edge free. This true matching-two certificate core is UNSAT for every uniform star multiplicity two through eight. Therefore the best next lemma is that every `H_x` has matching number at most one. Since `H_x` has no isolated vertices and is bipartite, that would make it a single star component and prove the needed local condition immediately.
