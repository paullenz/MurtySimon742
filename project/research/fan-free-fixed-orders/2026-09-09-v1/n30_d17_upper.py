#!/usr/bin/env python3
"""Fan-free exact-acceptance early-kernel closure for n=30, Delta=17, m=227..255."""
...
def main():
    ...
    for m in range(227,256):
        t=m-17*13;rec=base.run_scope(t);assert rec['final_survivors']==0,(m,rec['opens'])
        results[str(m)]={k:v for k,v in rec.items() if k not in ('duals','opens')};results[str(m)]['exact_dual_count']=len(rec['duals'])
    out={'schema':'fan-free-n30-d17-upper-v1','n':30,'Delta':17,'a':12,'b':17,'edge_range':[227,255],'results':results,'all_final_survivors_zero':True,'parameterized_source':str(p.relative_to(ROOT)),'dual_acceptance':'exact integer/rational checks inside established source-capacity implementation','status':'candidate fixed-order proof component; independent review open'}
