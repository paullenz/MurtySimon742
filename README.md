...
| `n=30` | Complete candidate: `e(G) <= 225`, equality exactly `K(15,15)`; frozen Fan-free reviewer v2 plus later candidate hand hardening of the entire Delta=16 upper-bound branch; seven new scalar formulas exclude 57/207 positive tight equality rows; finite equality dependencies and external review remain open |
...
At `m=225`, the same programme reconstructs the historical 272 residual rows from the 100 `Q>=18` profiles using at most three units of slack. Ledger equality excludes 61 rows; four zero-demand tight rows retain exact Hall certificates. The [new scalar continuation](project/research/n30/2026-09-11-m225-hall-continuation-v1/README.md) gives seven explicit formulas excluding **57 of the remaining 207 positive tight rows**, with minimum assigned exact gap `1/12`. A separate standard-library checker verifies every formula on the complete tight frontier. The other **150 positive rows** still require stronger preserved methods; they are not survivors of the full existing candidate proof. The 100-profile classification also remains a finite dependency. No full hand proof of the equality branch is claimed.
