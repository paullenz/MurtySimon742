...
| Row471 conditioned progress | Earlier branches `e_L=39,40` are exactly hand/verifier closed; [`row471-e41-v1`](project/research/general_n/2026-09-15-row471-e41-v1/README.md) closes `e_L=41`; new [`row471-e42-e43-common-pressure-v1`](project/research/general_n/2026-09-15-row471-e42-e43-common-pressure-v1/README.md) closes `e_L=42,43` by exact source-group common-pressure bounds `214<222` and `215<222`, while independently rechecking `41` at `213<222`. **Only `e_L=47` remains for row471** |
...
| Original synthetic sample | **708/713 rejected; five not rejected: 160,338,347,471,586.** Sample-level evidence only; row471 is still retained because `e_L=47` remains |
| Fresh-seed sample | **708/715 rejected; seven retained in a separate namespace.** Unchanged by the row108/row471 branch work |
...
| Row471 `42,43` verification | Exact standard-library replay passed locally with frozen canonical result SHA256 `b2f2b8f1f714eb11225c07d9a9595154a7c53523b5d8da74be8a2028c170cc15`. Dedicated remote CI is newly installed by this checkpoint and is **not yet claimed successful** |
| Documentation guards | Status-sync run34906766824 and reviewer-navigation run34906766838 both SUCCESS on the row108 commit. N30 package run34906766832 FAILED after an earlier README condensation removed explicit frozen PDF paths; those links were restored. Red-team-history guard run **34908428824 FAILED** because `START_HERE_FOR_REVIEWERS.md` was outside rather than inside the protected reviewer block; the later repair added it inside the block without weakening the guard. Standalone row471 note commit `2667a909...` also violated the paired-status publication order and remains recorded as a process failure |
| Active continuation | Attack the final row471 branch `e_L=47` with selected-label/destination compatibility plus the bridge's **ONE common residual neighbourhood per source**; the `charge_eta=0` receiver lower used for `41–43` is unavailable at `47`. In parallel carry exact common-pressure/source-incidence across the other retained originals |

**Checkpoint — 15 September 2026, `row471-e42-e43-common-pressure-v1`.** Inspected predecessor `2c640dcb2226bd4a6d056dc9da46cc4be9a53bc1`. The predecessor's independently verified `e_L=41` closure is preserved. A separate exact symmetry/nesting DP now closes `e_L=42,43` with source-group charge uppers `214,215` against receiver lower `222`, and independently gives `213<222` for `41`. **Row471 remains open only on `e_L=47`; original synthetic sample remains 708/713; canonical finite frontier remains 4,626 exclusions / 952 survivors / 3,632 whole-state closures.** External mathematical review remains OPEN.
...
- [Row471 `e_L=42,43` common-pressure split](project/research/general_n/2026-09-15-row471-e42-e43-common-pressure-v1/README.md): exact `rho=2` symmetry DP plus exact nested-high-excess DP gives source-group charge uppers `214,215<222`; `e_L=41` is independently rechecked at `213<222`; only `47` remains.
