...
The preserved research objective is a **genuinely independent proof of Erdős #742 / the Murty–Simon inequality**
...
The first focused session improved the internally checked candidate threshold from `7/12` to `250/429`: for `n>=6`, `Delta(G)>=250n/429` implies `e(G)<floor(n^2/4)`. The live strip is therefore `n/2<Delta(G)<250n/429`. The shared graph-to-profile bridge has now been reconstructed directly from raw criticality and replayed on 757 actual D2C graphs under all 2,722 legal selections at maximum-degree roots. It remains same-project internal verification. Any explicitly reauthorized programme must distinguish scalar/profile survivors from graph realizability and close or rigidify the strip without losing balanced complete-bipartite equality. A 24-focused-session go/no-go gate applies; the last saved counter is `7/24` in the 24 September 07:00 ledger; the midnight audit recorded the earlier `6/24`. Housekeeping does not award additional credit. See [the pivot note](project/research/general_n/2026-09-21-independent-742-pivot-v1/README.md), [23 September audit](project/research/general_n/2026-09-23-daily-red-team-audit-v1/DAILY_RED_TEAM_AUDIT.md), and [24 September audit](project/research/general_n/2026-09-24-daily-red-team-audit-v1/DAILY_RED_TEAM_AUDIT.md).
...
The four-centre branch is now completely classified and density-closed at Q3 antipodal-transversal scope: only parity-plane support is realizable, and every exactly-four-centre graph satisfies `m<=M(n)`. Exact order results give a unique order-19 graph with 66 edges and a unique order-20 extremal structure with 73 edges. Actual D2C controls invalidate matching-one, one-tree-component and exact block cancellation; the repaired quantitative excess bound `epsilon<=floor(s(r+q-2)/2)` (with `q=0` separate) fits inside the density budget with at least 11 units of slack. Five through eight centres and nontransversal codes remain open.

The raw certificate calculus passes 28,934 independent edge-deletion comparisons. The new graphs add no positive rigid-Hall fixture: all their checked maximum-degree roots have p=0, and this is proved for the balanced five-coordinate family. These are internal results awaiting external review, not a general eventual theorem. Raw realizability of the reduced nonparity-plane cores was the priority of this earlier branch, not an instruction to resume it; the conditional interface below remains preserved.
...
The regular independent-#742 research cadence and midnight audit are **paused by the user**. No automatic recovery or continuation is authorized. The historical daily-audit rules and earlier eventual-D2C work are preserved for review, not as a running schedule. Any later user-authorized research must inherit the latest audit constraints, retain `X_3` and balanced complete bipartite controls, and not revive superseded evidence.
...
**24 September 2026 — post-audit restoration and repository pause.** The later all-32 LP replay restored S<=14 at internal computer-assisted scope; the finite-row and branch evidence is linked in [CURRENT_STATE.md](CURRENT_STATE.md). The following midnight audit entry is retained as the earlier state, not the current freeze. The user subsequently stopped the cadence. See [housekeeping and unresolved evidence notes](project/maintenance/2026-09-24/REPORT.md).

**24 September 2026 - r=12 nonreproduction, graph-level corrections and launch audit.** The support-ten source count was corrected from 42 to 32 and all 6,386 committed source MILPs replayed cleanly, but a fresh execution of the load-bearing helper screen returned 32 timeouts rather than the saved 32 infeasibilities. `S<=14` is therefore frozen and the retained audited bound remains `S<=13`. Unrestricted crossed-supplement injectivity is refuted by actual D2C collisions, and the mistaken cap `x_i<=n-Delta-1` is withdrawn. The candidate threshold remains `250/429`; equality remains supported only through `S<=8`. Of 23 scheduled triggers, 18 contain substantive research, five met 40 minutes, four are compliant, four delivered zero research, and six remain unverified/incomplete. See the [full audit](project/research/general_n/2026-09-24-daily-red-team-audit-v1/DAILY_RED_TEAM_AUDIT.md).

**23 September 2026 - bridge, bounded-demand and launch audit.** The raw graph-to-profile bridge now has a standalone derivation and exact finite regression on 757 D2C graphs / 2,722 legal selections. Zero-demand rigidity and the repaired finite pipeline internally support the edge bound through `S<=13` and equality through `S<=8`; no threshold below `250/429` or general theorem is claimed. The audit found two truncated committed evidence files, preserved them, regenerated them from source and restored clean r=8/r=11 replays. It also reconciled all 23 scheduled triggers: eight scheduled sessions contain evidenced research, two are zero-research deliveries and thirteen have missing/unverified starts. See the [full audit](project/research/general_n/2026-09-23-daily-red-team-audit-v1/DAILY_RED_TEAM_AUDIT.md).
...
**24 September 2026 - independent-#742 audit.** The r=12 source count was corrected from 42 to 32, but the support-ten helper screen did not reproduce: the fresh same-script run returned 32 solver timeouts rather than the saved 32 infeasibilities. `S<=14` is frozen. Actual D2C controls also refuted unrestricted crossed-supplement injectivity, and a root-side notation swap invalidated `x_i<=n-Delta-1`; both failed claims remain preserved and are barred from the live chain. The exact report is [DAILY_RED_TEAM_AUDIT.md](project/research/general_n/2026-09-24-daily-red-team-audit-v1/DAILY_RED_TEAM_AUDIT.md).
