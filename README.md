...
**19 September source-premise repair follow-up.** The two premises isolated by the audit have now been checked at the exact level used by the finite source-tuple theorem. The corrected beta orientation gives a direct raw-criticality proof of distinct physical sources: reusing one physical `(x,y)` at two target fibres would force the fixed set `N_G(x)∩N_G(y)` to be two different singletons. Selected `(source,coordinate)` uniqueness is the one-representative-per-physical-edge convention, not a raw-witness uniqueness claim. The follow-up also found that the first graph-level checker had accidentally tested the alpha/matched-source orientation while labelling it beta; that evidence is explicitly withdrawn and the checker is corrected rather than silently replaced. The repaired checker now installs `X_3` as an executable hostile control and reconstructs the canonical selected/residual Hall ledger on actual small D2C graphs. See [source-premise repair](project/research/post_ms/2026-09-19-source-premise-graph-audit-v1/SOURCE_PREMISE_REPAIR.md) and the [corrected checker](project/research/post_ms/2026-09-19-source-premise-graph-audit-v1/check_source_premises_graph_level.py).
...
- [Source-premise graph-level repair and regression](project/research/post_ms/2026-09-19-source-premise-graph-audit-v1/SOURCE_PREMISE_REPAIR.md)

The two named source-tuple premises are now internally repaired and the corrected graph-level regression reaches the canonical selected/residual Hall ledger, with `X_3` installed as an executable negative control. The immediate priority is to extend that independent graph-derived regression into the rigid Hall witness quantities and exact pair-local `Ccap_P` ingredients; only after that interface survives should the **exact** one-code `Ccap_P` formula be combined with `(ONE)` and `(CROWD)` before any further global scalar relaxation.
