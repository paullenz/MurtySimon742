...
| Forced-core routing capacity | New [`forced-core-capacity-v1`](project/research/general_n/2026-09-15-forced-core-capacity-v1/README.md): low eligibility forces many `rho=1` sources onto one common selected-label core; exact labelled-routing receiver capacities are indivisible across those labels. This **excludes original rows160 and338** |
| Original synthetic sample | **713/713 rejected.** The accumulated necessary conditions now close this original laboratory; this remains sample-level evidence and does not change the canonical finite frontier |
| Singleton-destination theorem | [`singleton-destination-trap-v1`](project/research/general_n/2026-09-15-singleton-destination-trap-v1/README.md) independently excludes original synthetic rows347,471,586; its dedicated workflow **34910561258 SUCCESS** |
...
| Fresh-seed sample | **708/715 rejected; seven retained in a separate namespace.** Not yet scanned by the new forced-core package and unchanged here |
| Forced-core verification | Local exact replay excludes160 and338; row160 receiver capacities are `5,5,5,4,4,4,4` with best minimum three-label bin9 against required10; row338 has only17 candidate capacity for20 obligations. Frozen parsed-result SHA256 `8784209ee05bb1ec6cd6da559e8845dbfeee3e041a96dcce9f41360c6d12920e`. Dedicated remote CI is newly installed and not yet claimed successful |
...
| Active continuation | Scan the seven fresh survivors with the singleton and forced-core lemmas; generalize the forced-core partition mechanism beyond `rho=1`; then test any proved extension against the 952 canonical survivors without over-promotion |

**Checkpoint — 15 September 2026, `forced-core-capacity-v1`.** Inspected predecessor `66a49c5a3d61937e7d8a9e92ca7d7ee0937f5a7e`. For row160, ten `rho=1,q=3` sources are forced by eligibility onto the same three labels. Exact labelled-routing makes each receiver usable for at most one of those labels; the only relaxed candidate capacities are `5,5,5,4,4,4,4`, which cannot supply10 units to all three labels (exact best minimum9). For row338, ten `rho=1,q=2` sources force20 obligations but the only relaxed candidate capacities sum to17. **Both remaining original profiles are excluded, taking the original synthetic sample from711/713 to713/713. Canonical finite frontier remains 4,626 exclusions / 952 survivors / 3,632 whole-state closures. Fresh sample unchanged.** External mathematical review remains OPEN.
