...
| Threshold-Hall normal form pilot | [`threshold-hall-normal-form-v1`](project/research/general_n/2026-09-15-threshold-hall-normal-form-v1/README.md) exactly replays **92,922,346 profiles across 23/24 audited closures**. **22** states close using forced-core + pair-capacity + explicit incidence/pair-slot/target threshold cuts; state2812 leaves exactly three profiles, all excluded by the existing cost bound. State152 is deliberately outside the new replay claim. Discovery/audit hardening only; canonical frontier unchanged |
| Selected-incidence Ferrers theorem | [`selected-incidence-ferrers-v1`](project/research/general_n/2026-09-15-selected-incidence-ferrers-v1/README.md) proves that the generic selected-incidence circulation is **equivalent for all orders in the canonical relaxation** to explicit demand-suffix inequalities. Proof: capacitated Hall + monotone exchange + concavity across equal-demand blocks. Exhaustive independent small-case audit: **1,206,288 cases / 0 mismatches**. Structural normalization only; canonical frontier unchanged |
...
| Active continuation | Complete the exact 306-state remote coverage; preserve later witnesses and independently audit closures; attack compression of the remaining pair-slot and target-capacity Hall networks to monotone `rho`/`q` threshold sets; promote nothing until complete key coverage and the reviewed promotion gate succeed |
...
**Checkpoint — 15 September 2026, `threshold-hall-normal-form-v1`.** A separate exact replay of 23 of those 24 audited whole-state exclusions enumerates **92,922,346 admissible `q` profiles**. For 22 states, the old generic incidence, pair-Hall and target-Hall failure stages are reproduced exactly by explicit monotone threshold cut inequalities. State2812 leaves exactly three threshold-surviving profiles; the established exact cost bound excludes them with minimum target costs `74,74,75` against excess envelope `60`. State152 is intentionally not included in this new threshold-normal-form replay claim. **Mathematical/canonical status remains unchanged: 4,626 exclusions / 952 survivors / 3,632 whole-state closures; no closure is promoted here.** The next mathematical target is an exchange/compression theorem showing when arbitrary deficient source sets can be replaced by `rho`/`q` threshold sets.

**Checkpoint — 15 September 2026, `selected-incidence-ferrers-v1`.** The selected-incidence network is Ferrers: source `u` is eligible for label `i` exactly when `s_i<=rho_u`. Under the canonical row-eligibility bound, the original exact-row lower-bound circulation is feasible **iff**, for every distinct demand `d`, `sum_{s_i>=d}s_i <= sum_u min(q_u, |{i:d<=s_i<=rho_u}|)`. A capacitated-Hall proof reduces arbitrary label subsets to largest-demand prefixes by exchange and then to whole demand blocks by concavity. An independently structured exhaustive regression compares the original circulation, a lower-demand cover flow and the suffix test on **1,206,288** small instances with **zero mismatches**. This removes one generic max-flow stage mathematically across the relaxation, but **does not change the canonical frontier: 4,626 exclusions / 952 survivors / 3,632 whole-state closures.** External review remains OPEN.
...
- [Threshold-Hall normal form pilot](project/research/general_n/2026-09-15-threshold-hall-normal-form-v1/README.md): explicit monotone Hall/min-cut inequalities replace the three generic flow decisions on 22 of 23 replayed audited closures; state2812 reduces to three exact cost-bound profiles. State152 remains outside this new replay claim. This is structural audit hardening, not canonical promotion.
- [Selected-incidence Ferrers theorem](project/research/general_n/2026-09-15-selected-incidence-ferrers-v1/README.md): capacitated Hall, exchange and concavity prove that the original selected-incidence circulation is exactly characterized by demand-suffix inequalities at every order under the canonical eligibility bounds. Exhaustive 1,206,288-case regression gives zero mismatches. The remaining compression target is therefore pair-slot/target Hall, not selected incidence.
