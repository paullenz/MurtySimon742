...
| Singleton-destination theorem | New [`singleton-destination-trap-v1`](project/research/general_n/2026-09-15-singleton-destination-trap-v1/README.md): every `(q,rho)=(1,1)` source can point only to a `q<=1` destination; opposite arcs turn this into an unordered-pair Hall matching. This **excludes original synthetic rows347,471,586 as whole profiles** |
| Original synthetic sample | **711/713 rejected; only rows160 and338 are not rejected.** This remains sample-level evidence and does not change the canonical finite frontier |
| Row471 branch history | Exact branch packages for `e_L=39,40,41,42,43` remain preserved as independent evidence. The whole-profile singleton-destination obstruction now supersedes the need to attack `e_L=47` |
| Row471 `42,43` verification | Exact workflow **34909572352 SUCCESS** on commit `00c872dd...`; paired status-sync guard also passed. The frozen common-pressure result remains independent corroboration |
| Row108 source sharing | [`conditioned-source-sharing-v1`](project/research/general_n/2026-09-14-conditioned-source-sharing-v1/README.md) excludes original synthetic row108; remote workflow **34906766833 / job104185160249 SUCCESS** |
| Fresh-seed sample | **708/715 rejected; seven retained in a separate namespace.** Not scanned by the new original-only singleton package and unchanged here |
| Cardinality arc-slot cross-check | The stronger all-source necessary flow independently gives `127<129` on row347, `95<96` on row471 and `108<109` on row586, while rows160 and338 attain their full required outdegrees `81` and `101` and are deliberately retained as non-rejections |
| New verification state | Local exact replay of the singleton/cardinality package passed with canonical parsed-result SHA256 `9a89c420f82a0bd89eb8e99a34dd3109ace9c1829879e62e819d452f408313f7`. Dedicated remote CI is newly installed and is **not yet claimed successful** |
...
| Active continuation | Focus on the two remaining originals, **160 and338**. Both pass cardinality-only arc matching, so the next useful step is one actual selected-label family with destination compatibility plus the bridge's **ONE shared residual neighbourhood per source**. Separately scan the fresh namespace without mixing its row IDs |

**Checkpoint — 15 September 2026, `singleton-destination-trap-v1`.** Inspected predecessor `00c872dd6321aa4d7c99ada4a703644329733af7`. The selected-label/destination inequalities imply a short Hall obstruction for low-row sources. Rows347 and586 each have a sole `(q,rho)=(1,1)` source and no other `q<=1` destination; row471 has exactly two such sources and they are forced onto the same unordered pair, which would require forbidden opposite arcs. A broader exact cardinality-compatible pair-slot max-flow independently rejects all three. **Original synthetic sample improves from 708/713 to 711/713, leaving only160 and338. Canonical finite frontier remains 4,626 exclusions / 952 survivors / 3,632 whole-state closures. Fresh sample unchanged.** External mathematical review remains OPEN.
