...
**Canonical claim.** `e(G) <= 225, with equality exactly K(15,15)`.

**Canonical proof source.** `project/reviews/n30/2026-09-09-candidate-v1/PROOF.md`. The source is reproduced in full below; substantive mathematical changes must be made in the canonical proof first and then rebuilt into this edition.

---

# Murty–Simon at n=30: complete candidate proof
...
## 2. Published upper reduction

Use Fan's strict upper bound in the form already used and cited in the n=29 candidate:

```text
e(G) < n^2/4 + [n^2-(81/5)n+56]/320.
```

At `n=30` the right-hand side is exactly

```text
7247/32 = 226.46875.
```

Therefore a counterexample to `e(G)<=225` can have only

```text
m=226.
```

Equality at `m=225` is handled separately; no edge is deleted and then assumed to leave an edge-critical graph.
...
9. the use and exact hypotheses of the cited published Fan and dominating-edge reductions.
