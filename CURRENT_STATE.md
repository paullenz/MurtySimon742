...
CHECKPOINT CLASS: `ONE_CODE_R1_K2_D1_REVERSE_HOLE_BLOCK_2026_09_20`
...
LAST VERIFIED RESULT: `Same-session conditional continuation. The U--U singleton degree-sum lemma was independently replayed. Any K-free escape touching even one selected witness z_h is forced by the existing raw B-edge theorem into N(t) cap N(h)={z_h}, hence is Y-anticomplete and anticomplete to the entire K-heavy reservoir; only fully K/W_s-free exceptions F0 can repair K-heavy internal or K-heavy--Y edges. Internal-edge U--U certificates and K-heavy--Y U--U certificates share one graph-fixed (source,witness) incidence budget. On the exact low-k ray, retaining actual K-heavy/F0 sizes gives (1+2f/p)E_R+(2r/p)E_F+3A+yq>=r(r-1). Every W_s-free non-K-heavy escape has exact H/Y/E deficit p-4+epsilon+d_K. Additive physical accounting sharpens the global asymptotic defect to liminf D_phys/p^2 >= (7-sqrt(33))/8=0.156929669..., while an R/F0-only endpoint costs at least 1/4. Therefore any sequence approaching the current coarse lower envelope must contain linearly many D1 vertices: W_s-free escapes with exactly one K-neighbour. At J2=empty, a D1 vertex t adjacent q_j has only two raw-criticality outcomes. Forward gives N(t) cap N(b)={q_j}, so t is Y-anticomplete and anticomplete to all K-heavy escapes. Reverse gives an injective matched head h(t) with N(q_j) cap N(h(t))={t}. If T is the set of D1 q_j-neighbours and v are reverse, their distinct matched heads miss all other vertices of T, producing at least v(|T|-1) H--D1 holes plus 2v H--W_s holes. Thus a large q_j-neighbour D1 population is expensive in either orientation; the remaining permissive endpoint must push mass toward q_j-nonneighbours or a strongly unbalanced orientation split.`

UNPRESERVED WORK: `None. ONE_CODE_R1_K2_EXCEPTION_ORIENTATION_CONSERVATION.md, ONE_CODE_R1_K2_F0_SECTOR_DEFICIT.md, ONE_CODE_R1_K2_GLOBAL_QUADRATIC_DEFECT.md, ONE_CODE_R1_K2_WEIGHTED_LEDGER_GAP.md, ONE_CODE_R1_K2_EXCEPTION_COEFFICIENT_REFINEMENT.md, ONE_CODE_R1_K2_RF0_COUPLED_MINIMUM.md, ONE_CODE_R1_K2_ADDITIVE_PARTITION_MINIMUM.md, ONE_CODE_R1_K2_D1_QJ_SPOKE.md, ONE_CODE_R1_K2_D1_QJ_REVERSE_HOLE_BLOCK.md, and this handoff are committed. No finite scan was used for the new results.`

DEFERRED ADMIN: `README remains deliberately at the independently audited public checkpoint. Do not promote the same-session k=2 exception/global-defect/D1 claims before the next daily hostile audit. Historical invalidated/superseded claims and CI failures remain preserved.`

NEXT ACTION: `First hostile-replay OC-INERT, the shared I+J_U pair budget, J_X<=yq, coefficient-sensitive CR-MAIN, D-sector distinct-hole conversion, AP-NONR additivity, and both D1 q_j orientation notes. If they survive, split D1 into q_j-nonneighbours, forward q_j-neighbours, and reverse q_j-neighbours. Retain the forward Y/R hole blocks and reverse H--T rectangle v(|T|-1)+2v. Combine them with the exact D1 sector identity alpha+beta+gamma=p-3+epsilon and K-heavy exception conservation. If the weighted optimizer moves mass to q_j-nonneighbours, attack those vertices by the missing residual-hub edge/private-coordinate geometry. Keep exact currencies E_U, Z_X, Z_Y and M_U separate.`
...
R1 PREDECESSOR: `At residual dimension one, the common hub q_j, forced beta fan, W_s--Y and W_s--U_bar(d) anticompleteness, K--H anticompleteness, physical selected-witness repricing, and orientation-covering theorem remain the active conditional spine. The exact low-k parameter escape c=p=lambda=t, y=t-1, u=x=t+1, k=2, n=5t+2 survives the predecessor aggregate score/rooted and pair-local scalar gates and is a method family, not a graph construction.`
...
R1 K2 D1 HUB: `At J2=empty, N_A(q_j)=K. If D1 vertex t with N_K(t)={a}, N_Ws(t)=empty sees q_j, forward orientation forces witness b and N(t) cap N(b)={q_j}, hence N_Y(t)=empty and t--R empty. Reverse excludes K and Y and forces h_i in H with N(q_j) cap N(h_i)={t}. For T all D1 q_j-neighbours and reverse subset size v, the reverse heads are injective and miss every other T vertex, giving Z_X>=v(|T|-1)+2v from H--T and H--W_s alone.`
...
`-> D1 isolated as live cheap endpoint`
`-> D1 q_j forward/reverse raw-criticality dichotomy`
`-> reverse D1 H--T hole rectangle`
`-> live D1 q_j-nonneighbour / weighted-rooted attack`.

## Session ledger — current invocation

**Invocation start:** 17:26:52 BST.  
**Planned preservation cutoff:** 17:55:38 BST.
...
10. synthesized and optimized the five-class global quadratic defect, then reconstructed exact weighted score/rooted ceilings and exposed the remaining coefficient gap rather than claiming closure;
11. hostile-replayed the new exception-orientation chain and sharpened it to `(1+2f/p)E_R+(2r/p)E_F+3A+yq>=r(r-1)` by retaining actual F0 and K-heavy sizes;
...
14. began the literal D1 attack: at J2=empty, criticality of a D1--q_j edge gives either a forward Y/R-anticomplete outcome or an injective reverse matched-head witness;
15. hostile-replayed that D1 orientation exhaustion and strengthened the reverse side to an almost-complete missing H--T rectangle of size `v(|T|-1)` plus `2v` H--W_s holes.

**Live unfinished line:** first replay the newest D1 rectangle and coefficient synthesis independently; then quantify the three-way D1 split (q_j-nonneighbour / forward / reverse) inside the exact weighted rooted ledger. If mass moves to q_j-nonneighbours, attack that literal geometry next.

**Stop reason:** final preservation checkpoint after fifteen tightly connected mathematical/audit units. No mathematical blocker; the next coherent D1 structural line is preserved for immediate continuation without overlapping the next scheduled research invocation.
