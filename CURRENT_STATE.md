...
**15 September 2026 — checkpoint `restore-redteam-readme-history-guard-fix-v1`.** Inspected predecessor: `0484cb261f97b8dacf5acbf6b1efec41a98392c9`. The reviewed 2,655-key relational promotion and restored hostile/red-team history are preserved unchanged. The first strengthened README guard run `34908428824` failed because `START_HERE_FOR_REVIEWERS.md` was outside rather than inside the protected reviewer block; this repair duplicates that navigation inside the protected block without weakening the guard, and the failed run remains recorded as failed. **Mathematical status unchanged: canonical finite frontier remains 4,626 exclusions / 952 survivors / 3,632 whole-state closures; fixed-order/general candidates and row108/row471 status are unchanged.** External review remains OPEN.
...
Strengthened guard run **34908428824** remains FAILED. Its failure was navigational, not mathematical: `START_HERE_FOR_REVIEWERS.md` existed at the README top but not inside the protected `REVIEW-MATERIALS` block. This repair adds that link inside the protected block and does not weaken the red-team-history checks.
