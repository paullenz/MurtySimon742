...
p = Path('README.md')
s = p.read_text()
marker = '<!-- STANDALONE-PAPERS:START -->'
banner = '''<!-- STANDALONE-PAPERS:START -->
...
'''
if marker not in s:
    needle = '## Current status\n\n'
    if needle not in s:
        raise SystemExit('current-status insertion point not found')
    s = s.replace(needle, needle + banner, 1)

review_marker = '<!-- ACTIVE-STANDALONE-PAPERS:REVIEW -->'
review_block = '''<!-- ACTIVE-STANDALONE-PAPERS:REVIEW -->
...
if review_marker not in s:
    needle = '### Fixed-order papers and packages\n'
    if needle not in s:
        raise SystemExit('review-material insertion point not found')
    s = s.replace(needle, review_block + needle, 1)
p.write_text(s)

c = Path('CURRENT_STATE.md')
t = c.read_text()
old = '**README SYNC:** pending one-time self-removing workflow in this transaction; it will add a prominent root-README paper banner and reviewer links without rewriting historical content.'
new = '**README SYNC:** root README paper banner and reviewer links synchronized; historical content preserved.'
if old in t:
    t = t.replace(old, new)
c.write_text(t)
